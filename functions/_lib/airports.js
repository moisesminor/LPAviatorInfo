...
async function fetchWikipediaAirportPhoto(airportName) {
  ...
  try {
    ...
    const response = await fetch(url, {
      headers: { 'User-Agent': 'AviatorInfo/1.0 (https://github.com/NetinhoSZ/LPAviatorInfo)' }
    });
...
export async function getAllAirportsRaw() {
  const now = Date.now();
...
export async function getAirportsListByCountry(countryName) {
  const now = Date.now();
...
export async function findAirportCoordsByCode(code) {
...
async function fetchAirportDetailsByIcao(icao, apiToken) {
...
export async function resolveCountryAirports(countryName, apiToken) {
  const now = Date.now();
  ...
  const resolved = await mapWithConcurrency(airportsBase, PHOTO_RESOLUTION_CONCURRENCY, async (airport) => {
    const details = await fetchAirportDetailsByIcao(airport.icao, apiToken);
...
export async function buildAirportsResponse(countryName, searchParams, apiToken) {
  const offset = Number(searchParams.get('offset')) || 0;
  const limit = Math.min(Number(searchParams.get('limit')) || 20, 50);
  const search = normalizeText(searchParams.get('search') || '');
  let airports = await resolveCountryAirports(countryName, apiToken);
...
export function normalizeFlightData(flight = {}) {
